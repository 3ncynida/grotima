                ...
                <form method="GET" action="{{ route('data.total_sales_per_day') }}">
                    <div class="form-row">
                        <div class="form-group col-md-2">
                            <label for="dropshipper_id">Seller</label>
                            <select name="dropshipper_id" id="dropshipper_id" class="form-control">
                                <option value="">None</option>
                                @foreach ($dropshippers as $dropshipper)
                                    <option value="{{ $dropshipper->id }}"
                                        {{ request('dropshipper_id') == $dropshipper->id ? 'selected' : '' }}>
                                        {{ $dropshipper->nama_dropshipper }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-2">
                            ...
                            <select name="marketplace_id" id="marketplace_id" class="form-control">
                                ...
                                @foreach ($marketplaces as $marketplace)
                                    <option value="{{ $marketplace->id }}"
                                        {{ request('marketplace_id') == $marketplace->id ? 'selected' : '' }}>
                                        {{ $marketplace->marketplace_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-2">
                            ...
                            <select name="ekspedisi_id" id="ekspedisi_id" class="form-control">
                                ...
                                @foreach ($ekspedisis as $ekspedisi)
                                    <option value="{{ $ekspedisi->id }}"
                                        {{ request('ekspedisi_id') == $ekspedisi->id ? 'selected' : '' }}>
                                        {{ $ekspedisi->ekspedisi_name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-2">
